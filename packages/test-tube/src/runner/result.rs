    ...
    R: prost::Message + Default,
{
    ...
    fn try_from(res: ExecTxResult) -> Result<Self, Self::Error> {
        ...
        let events = res
            .events
            .into_iter()
            .map(|e| -> Result<Event, cosmrs::tendermint::Error> {
                Ok(Event::new(e.kind).add_attributes(
                    e.attributes
                        .into_iter()
                        .map(|a| -> Result<Attribute, cosmrs::tendermint::Error> {
                            Ok(Attribute {
                                key: a.key_str()?.to_string(),
                                value: a.value_str()?.to_string(),
                            })
                        })
                        .collect::<Result<Vec<Attribute>, cosmrs::tendermint::Error>>()?,
                ))
            })
            .collect::<Result<Vec<Event>, cosmrs::tendermint::Error>>()?;
